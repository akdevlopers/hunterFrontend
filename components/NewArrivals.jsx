...
export default function NewArrivals() {
  const { allApiProducts, isHomeLoading } = useShop();

  const isLoading = isHomeLoading || (!allApiProducts || allApiProducts.length === 0);

  const productsToDisplay = allApiProducts.slice(0, 8).map((item) => ({
    id: item.id,
    slug: item.slug || item.id,
    name: item.name,
    category: item.category_id === 21 ? "Pant" : item.category_id === 15 ? "Hoodies" : item.category_id === 11 ? "Shirt" : item.category_id === 4 ? "T-Shirt" : "Streetwear",
  ...
  return (
    <section
      id="new-arrivals"
      className="max-w-[1550px] mx-auto px-4 sm:px-6 lg:px-10 xl:px-12 py-8 sm:py-10 lg:py-12 border-t border-gray-100"
    >
      {/* Header */}
          ...
          <ScrollReveal key={item.id} direction="up" delay={50 * (idx + 1)}>
            <Link href={`/products/${item.slug || item.id}`} className="group cursor-pointer flex flex-col transition-all duration-500 hover:-translate-y-2">
              <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-[#f6f6f6] flex items-center justify-center">
                ...
                {/* View Product Button (Shows On Hover / Tap) */}
